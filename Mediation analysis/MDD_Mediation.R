...
File_path = paste(Project_path,'Mediation analysis/MDD_tbl.csv',sep = '/')
df <- as.data.frame(fread(File_path))
colnames(df)[2] = 'PRS_MDD'

Y.names <- colnames(df[185:227]) # 227:269
X.names<- colnames(df[2]) # mdd score
M.names<- colnames(df[c(3:184)]) # 38:218
Y.n = length(Y.names)
X.n = length(X.names)
M.n = length(M.names)
...
save_path = paste(Project_path,'Mediation analysis/Result',sep = '/')
